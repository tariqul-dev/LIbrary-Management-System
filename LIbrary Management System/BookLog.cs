...
namespace LIbrary_Management_System
{
    public partial class BookLog : Form
    {
        ...
        private void BookLog_Load(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("select * from Book_log;", con);
                cmd.ExecuteNonQuery();
                ...
                BS.DataSource = dt;
                BookLogDataGridView.DataSource = BS;
                sda.Update(dt);
        ...
        private void FilterComboBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("select * from Book_log where log_action like '%" + FilterComboBox.Text + "%';", con);
                    cmd.ExecuteNonQuery();
                    DataTable dt = new DataTable();
                    SqlDataAdapter sda = new SqlDataAdapter();
                    sda.SelectCommand = cmd;
